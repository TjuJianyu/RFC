...
if methods == 'irmv2':
	...
	for penalty_anneal_iters in [0, 50, 100, 150, 200, 250]:
		for penalty_weight in [p//10, p//2, p, p*5, p*10]:
			...
			flags.steps = 701 
			flags.dataset = 'coloredmnist025'
			results, logs = rich_main(flags)
			np.save('log/final/nll_%s_%d_%d.npy' %  \
	...
	for penalty_anneal_iters in [  250,0, 50, 100,150, 200,]:
		...
		for penalty_weight in [ p*100, p*50]:
	...
	for penalty_anneal_iters in [0, 50, 100, 150, 200, 250]:
		...
		for penalty_weight in [p*0.96, p*0.94]:
			t = time.time()
			flags = FLAGS()
			...
			results, logs = rich_main(flags)
			np.save('log/final/nll_%s_%d_%.6f.npy' %  \
				(methods, penalty_anneal_iters, penalty_weight), logs)
			print(time.time() - t)
	...
	for penalty_anneal_iters in [0, 50, 100, 150, 200, 250]:
		...
		#for penalty_weight in [p/10, p/2]:
	...
	for penalty_anneal_iters in [0, 50, 100, 150, 200, 250]:
		...
		for penalty_weight in [0.6, 0.7]:
			...
			flags.steps = 701 
			results, logs = rich_main(flags)
			np.save('log/final/nll_%s_%d_%.1f.npy' %  \
				(methods, penalty_anneal_iters, penalty_weight), logs)
			print(time.time() - t)

elif methods == 'erm':
	#for l2_regularizer_weight in [0.0001, 0.0005, 0.001, 0.005, 0.01]:
	for l2_regularizer_weight in [0.00001, 0.00005]:
	...
	for l2_regularizer_weight in [0.00001, 0.00005]:
		...
		flags.methods = 'erm'
		flags.rep_init = 'none'
		flags.dataset = 'coloredmnist025gray'
		flags.lr = flags.rich_lr 
		flags.anneal_val = 0
		flags.penalty_anneal_iters = 0
		flags.penalty_weight = 0
