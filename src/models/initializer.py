...
def initialize_model(config, d_out, is_featurizer=False):
    ...
    # whether freeze the weights of representation
    freeze_featurizer = False 
    if hasattr(config, 'freeze_featurizer') and config.freeze_featurizer is True:
        if config.model not in ('resnet18', 'resnet34', 'resnet50', 'resnet101', 'wideresnet50', 'densenet121'):
...
def initialize_torchvision_model(name, d_out, freeze_featurizer, **kwargs):
    import torchvision
