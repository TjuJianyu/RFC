...
from dataset.camelyon17_dataset import RfcCamelyon17Dataset
from get_loader import get_train_loader, get_eval_loader
...
def main():
    ...
    #rfc
    parser.add_argument('--rfc_groups_dir', required=True,
                            help='The directory where [dataset]/data can be found (or should be downloaded to, if it does not exist).')
    ...
    if config.dataset == 'camelyon17':
        ...
        #     print(groups[:,i].mean())
        #     full_dataset._metadata_array[:,0] = torch.LongTensor(groups[:,i])
        ...
        full_dataset = RfcCamelyon17Dataset(
                    version=config.version,
    ...
    for split in full_dataset.split_dict.keys():
        ...
        if split == 'train':
            datasets[split]['loader'] = get_train_loader(
                ...
                n_batches = len(datasets[split]['dataset']) // full_dataset.n_groups // config.batch_size,
                **config.loader_kwargs)
            #print(len(datasets[split]['loader'] ),len(datasets[split]['dataset']) // full_dataset.n_groups // config.batch_size)
        else:
            datasets[split]['loader'] = get_eval_loader(
                ...
                batch_size=config.batch_size,
                **config.loader_kwargs)
            print(len(datasets[split]['loader'] ))
        # Set fields
