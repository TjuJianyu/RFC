...
from torch.utils.data.sampler import WeightedRandomSampler, SubsetRandomSampler,RandomSampler
from wilds.common.utils import get_counts, split_into_groups

def get_train_loader(loader, dataset, batch_size,
        uniform_over_groups=None, grouper=None, distinct_groups=True, n_groups_per_batch=None, n_batches=None, **loader_kwargs):
    """
    ...
    assert n_batches is None or (len(dataset) // batch_size >= n_batches)

    if loader == 'standard':
        if uniform_over_groups is None or not uniform_over_groups:
            if n_batches is not None:
                sampler = RandomSampler(dataset, n_batches*batch_size, replacement=False)
            return DataLoader(
                dataset,
                shuffle=True if n_batches is None else False, # Shuffle training dataset
                sampler=None if n_batches is None else sampler,
            ...
            #print(n_batches, n_batches*batch_size)
            # Replacement needs to be set to True, otherwise we'll run out of minority samples
            if n_batches is not None:
                sampler = WeightedRandomSampler(weights, n_batches*batch_size, replacement=True)
            else:
            ...
            #     shuffle=True, # The WeightedRandomSampler already shuffles
            #     sampler=None,
            #     collate_fn=dataset.collate,
            #     batch_size=batch_size,
            #     **loader_kwargs)
        ...
        if n_batches is not None:
            batch_sampler.num_batches = n_batches
