...
from wilds.common.metrics.all_metrics import Accuracy,multiclass_logits_to_pred

from losses import RFCmtAccuracy
...
class IIDCamelyon17Dataset(WILDSDataset):
    """
...
class SynCamelyon17Dataset(WILDSDataset):
    """
    The CAMELYON17-WILDS histopathology dataset.
    This is a modified version of the original CAMELYON17 dataset.
    ...
    def __init__(self, version=None, root_dir='data', rfc_meta_dir='',group_method='balance', download=False, split_scheme='official', predicts=None):
        self._version = version
        self._data_dir = self.initialize_data_dir(root_dir, download)
        self._original_resolution = (96,96)
        print('rfc_meta_dir',rfc_meta_dir)
        self.rfc_meta_dir = rfc_meta_dir
        #rfc meta 
        rfc_groups = pd.read_csv(os.path.join(rfc_meta_dir, 'groups.csv'),header=None).values
        pseudolabels = pd.read_csv(os.path.join(rfc_meta_dir, 'pseudolabels.csv'),header=None).values
        print('pseudolabels shape', pseudolabels.shape)
        
        #pseudolabels_distribution = pd.read_csv(os.path.join(rfc_meta_dir, 'pseudolabel_dists.csv'),header=None).values
        self.n_groups = rfc_groups.shape[1]
        #pseudolabels_distribution = pseudolabels_distribution.reshape((pseudolabels_distribution.shape[0],self.n_groups,-1))
        
        #print('pseudolabels_distribution',pseudolabels_distribution, pseudolabels_distribution.shape)
        # only support 2 class correct/incorrect
        print('row rfc groups',rfc_groups)


        for i in range(self.n_groups):
            rfc_groups[:,i] -= 2*i
            rfc_groups[:,i] = 1 - rfc_groups[:,i]
        
        if group_method == "old": 
            print('rfc groups',rfc_groups)
            rfc_groups = rfc_groups.prod(axis=1)
            print(rfc_groups.mean())

        elif group_method == 'balance':
            rfc_groups = (rfc_groups==1).sum(axis=1)
            rfc_groups = ((rfc_groups==0) | (rfc_groups == self.n_groups)).astype(int)
        elif group_method == 'non':
            rfc_groups = np.zeros(len(rfc_groups)).astype(int)
        else:
        ...
        #         rfc_groups[:,i] *= size 
        #         size *= 2
        #     rfc_groups = rfc_groups.sum(axis=1)


        #full_dataset._metadata_array[:,0] = torch.LongTensor(torch.LongTensor((1-groups[:,0]) * (3-groups[:,1])))
        
        #full_dataset._metadata_array[:,0] = torch.LongTensor(torch.LongTensor(groups.prod(axis=1)))
        ...
        self._y_array = torch.LongTensor(pseudolabels)
        self._y_size = 1
        self._n_classes = 2
    ...
    def eval(self, y_pred, y_true, metadata, prediction_fn=multiclass_logits_to_pred):
        """
        Computes all evaluation metrics.
        Args:
            - y_pred (Tensor): Predictions from a model. By default, they are predicted labels (LongTensor).
        ...
        Output:
            ...
            - results_str (str): String summarizing the evaluation metrics
        """
        metric = RFCmtAccuracy(prediction_fn=prediction_fn)
        return self.standard_group_eval(
            metric,
            self._eval_grouper,
            y_pred, y_true, metadata)
...
class RfcCamelyon17Dataset(WILDSDataset):
    """
    The CAMELYON17-WILDS histopathology dataset.
    This is a modified version of the original CAMELYON17 dataset.
    ...
    def __init__(self, version=None, root_dir='data', rfc_meta_dir='.', download=False, split_scheme='official', predicts=None):
        self._version = version
        self._data_dir = self.initialize_data_dir(root_dir, download)
        self._original_resolution = (96,96)
        self.rfc_meta_dir = rfc_meta_dir
        ...
        # concatenate metadata_df
        metadata_df = []
        slide_count = 0
        for i in range(self.n_groups):
            tmp = self._metadata_df.copy()
            tmp['slide'] += slide_count
            slide_count += len(np.unique(tmp['slide'].values))
            metadata_df.append(tmp)
        self._metadata_df=(pd.concat(metadata_df, ignore_index=True ))
        ...
        self._y_size = 1
        self._n_classes = 2
        #print(len(self._y_array))
        #concatenate y_array
        
       # self._y_array = torch.LongTensor(np.concatenate([self._metadata_df['tumor'].values for i in range((self.n_groups))]))
        ...
        # rfc groups instead of the original hospital groups 
        groups = np.concatenate([rfc_groups[:,i] for i in range(rfc_groups.shape[1])])
        #print(len(groups))
        #print(rfc_groups.shape)
        #print(self._y_array.shape)
        self._metadata_array = torch.stack(
            (#torch.LongTensor(centers),
    ...
    def get_input(self, idx):
       ...
       idx = idx % (len(self._y_array) // self.n_groups)
