...
def run_epoch(algorithm, dataset, general_logger, epoch, config, train, unlabeled_dataset=None):
    ...
    for labeled_batch in batches:
        ...
        #print(y_pred)
        #print(config.process_outputs_function)
    ...
    print(epoch_y_pred,epoch_y_true,epoch_metadata)
...
def evaluate(algorithm, datasets, epoch, general_logger, config, is_best, skip_train=True):
    algorithm.eval()
    torch.set_grad_enabled(False)
    for split, dataset in datasets.items():
        ...
        if not (skip_train and split == 'train'): 
            save_pred_if_needed(epoch_y_pred, dataset, epoch, config, is_best, force_save=True)


def evaluate_logits(algorithm, datasets, epoch, general_logger, config, is_best, skip_train=True):
    algorithm.eval()
    torch.set_grad_enabled(False)
    for split, dataset in datasets.items():
        if (not config.evaluate_all_splits) and (split not in config.eval_splits):
        ...
        for batch in iterator:
            batch_results = algorithm.evaluate(batch)
            epoch_y_true.append(detach_and_clone(batch_results['y_true']))
            logits = detach_and_clone(batch_results['y_pred'])
            if config.process_outputs_function is not None:
                y_pred = process_outputs_functions[config.process_outputs_function](logits)
            else:
                y_pred = logits
            epoch_y_logits.append(logits)
            epoch_y_pred.append(y_pred)
            epoch_metadata.append(detach_and_clone(batch_results['metadata']))

        epoch_y_pred = collate_list(epoch_y_pred)
        epoch_y_true = collate_list(epoch_y_true)
        epoch_y_logits = collate_list(epoch_y_logits)
        epoch_metadata = collate_list(epoch_metadata)
        results, results_str = dataset['dataset'].eval(
            epoch_y_pred,
            epoch_y_true,
        ...
        if not (skip_train and split == 'train'): 
            save_pred_if_needed(epoch_y_pred, dataset, epoch, config, is_best, force_save=True)
            save_logits_if_needed(epoch_y_logits, dataset, epoch, config, is_best, force_save=True)
...
def save_logits_if_needed(y_pred, dataset, epoch, config, is_best, force_save=False):
    if config.save_pred:
        prefix = get_pred_prefix(dataset, config)
        if force_save or (config.save_step is not None and (epoch + 1) % config.save_step == 0):
            save_pred(y_pred, prefix + f'epoch:{epoch}_logits')
        if (not force_save) and config.save_last:
            save_pred(y_pred, prefix + f'epoch:last_logits')
        if config.save_best and is_best:
            save_pred(y_pred, prefix + f'epoch:best_logits')
